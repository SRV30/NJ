...
const ManageJewelleryType = () => {
  ...
  const { loading, error, success, categories } = useSelector((state) => state.jewellery);
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
      dispatch(updateJewelleryCategory({ id: editId, formData }));
    } else {
      dispatch(addJewelleryCategory(formData));
    }
  };
  ...
  const confirmDelete = () => {
    if (categoryToDelete) {
      dispatch(deleteJewelleryCategory(categoryToDelete));
  ...
  useEffect(() => {
    ...
    if (success) {
      showJewelryToast(
        editId
          ? "Jewellery category updated successfully!"
          : "Jewellery category added successfully!",
        "success"
      );
      setName("");
      setImage(null);
  ...
  const cardVariants = {
    hover: { scale: 1.05, boxShadow: "0 10px 20px rgba(255, 193, 7, 0.2)", transition: { duration: 0.3 } },
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-100 via-amber-50 to-amber-100 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800 py-16 px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        >
          {editId ? "Refine Jewelry Category" : "Craft New Jewelry Category"}
        </motion.h2>
          ...
          <div className="flex gap-4">
            ...
            >
              {loading ? (
                ...
                editId ? "Update Jewelry" : "Add Jewelry"
              )}
            </motion.button>
            {editId && (
              <motion.button
              ...
              {categories.map((category) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-amber-800 dark:text-amber-100 font-serif font-medium text-lg capitalize">
                      {category.name}
                    </span>
                  </div>
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this jewelry category? This action cannot be undone."
      />
    </div>
  );
};

export default ManageJewelleryType;